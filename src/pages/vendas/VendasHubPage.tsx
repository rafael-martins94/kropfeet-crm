...
export default function VendasHubPage() {
  const contagens = useAsync(() => vendasService.contarPorRegiao(), []);
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {regioes.map((regiao) => {
          const total = contagens.data?.[regiao.key];
          return (
            <Link
              key={regiao.key}
              to={regiao.to}
            ...
            >
              ...
              <div className="mt-auto flex items-baseline gap-2 border-t border-line/80 pt-4">
                <span className="font-numeric text-2xl font-semibold tabular-nums text-ink">
                  {contagens.loading ? "…" : (total ?? 0).toLocaleString("pt-BR")}
                </span>
                <span className="text-xs text-ink-soft">ordens</span>
              </div>
            </Link>
          );
        })}
      ...
      {contagens.error ? (
        <p className="mt-4 text-sm text-red-700">Erro ao carregar contagens: {contagens.error.message}</p>
      ) : null}
    </div>
  );
}
