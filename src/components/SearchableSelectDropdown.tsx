...
/** Dropdown com campo de busca para filtrar opções por texto (accent-insensitive). */
export function SearchableSelectDropdown({
  ...
  const inner = (
    <>
      ...
      {open ? (
        <div
          className="absolute left-0 right-0 top-[calc(100%+6px)] z-[60] overflow-hidden rounded-xl border border-line bg-surface shadow-xl ring-1 ring-black/[0.06]"
          role="presentation"
        >
  ...
  return (
    <div ref={rootRef} className={cn("relative w-full sm:w-auto sm:min-w-[16rem]", className)}>
      {label ? (
