-- Permite preço sem moeda explícita (inferida pela região do local).
-- Atualiza publicar_vitrine para resolver EUR/BRL conforme tipo_regiao.
...
CREATE OR REPLACE FUNCTION public.moeda_venda_item_estoque(
  p_moeda_venda text,
  p_tipo_regiao public.tipo_regiao_enum
)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT COALESCE(
    NULLIF(trim(p_moeda_venda), ''),
    CASE p_tipo_regiao
      WHEN 'europa' THEN 'EUR'
