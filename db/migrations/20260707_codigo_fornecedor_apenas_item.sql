-- Código do produto no fornecedor fica apenas em itens_estoque.codigo_fornecedor.

UPDATE public.itens_estoque ie
SET codigo_fornecedor = mp.codigo_fabricante
FROM public.modelos_produto mp
WHERE mp.id = ie.id_modelo_produto
  AND NULLIF(trim(ie.codigo_fornecedor), '') IS NULL
  AND NULLIF(trim(mp.codigo_fabricante), '') IS NOT NULL;

UPDATE public.itens_estoque ie
SET codigo_fornecedor = oc.codigo_pelo_fornecedor
FROM public.ordens_compra oc
WHERE oc.id_item_estoque = ie.id
  AND NULLIF(trim(ie.codigo_fornecedor), '') IS NULL
  AND NULLIF(trim(oc.codigo_pelo_fornecedor), '') IS NOT NULL;
...
ALTER TABLE public.ordens_compra
  DROP COLUMN IF EXISTS codigo_pelo_fornecedor;

ALTER TABLE public.modelos_produto
  DROP COLUMN IF EXISTS codigo_fabricante;
